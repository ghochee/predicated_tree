...
void test_tree_invariants(raw_tree<T> &root) {
    ...
    REVERSE_EQ(root.postlbegin(), root.postlend(), root.prerbegin(),
               root.prerend());
    REVERSE_EQ(root.inlbegin(), root.inlend(), root.inrbegin(), root.inrend());

    std::vector<typename raw_tree<T>::value_type> elements;
    auto r = root.template detach<side::right>();
    std::copy(root.prelbegin(), root.prelend(),
              std::inserter(elements, elements.end()));
    std::copy(r.prelbegin(), r.prelend(),
              std::inserter(elements, elements.end()));
    root.template replace<side::right>(std::move(r));
